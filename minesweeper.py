...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Initialize an empty set to add neighbours
        neighbours = set()

        # Loop over all the neighbouring cells
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    ...
                    # Ignore if a cell is a mine or a safe and update the count
                    if (i, j) in self.mines:
                        count -= 1
                    if (i, j) not in self.safes and (i, j) not in self.mines:
                        neighbours.add((i, j))
        
        # Create a new sentence from given knowledge
        ...
        # Infer any new sentences using the subset method and add it to the KB
    ...
    def make_random_move(self):
        ...
        """
        # Keep track of all the moves which have not been made
        available_moves = []
        for i in range(self.height):
            for j in range(self.width):
        ...
        # Pick a move randomly from the available moves
